...
// Import DTOs later
import { ChangeNCPDto, ClaimDto, DepositDto, WithdrawAllDto, WithdrawAllWithPidDto, WithdrawDto, WithdrawRequestDto } from 'src/dto/wonder-staking-dto';
...
export class WonderStakingController {
    ...
    async withdrawRequest(@Body() withdrawRequestDto: WithdrawRequestDto) {
      try {
        ...
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            error: 'There was a problem with the withdrawal request in Wonder Staking',
            details: error.message,
          },
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
    ...
    @Post('withdrawAll')
    async withdrawAll(@Body() withdrawAllDto: WithdrawAllDto) { // Assuming the existence of WithdrawAllDto
      try {
        ...
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            error: 'There was a problem with the withdraw all operation in Wonder Staking',
            details: error.message,
          },
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
    ...
    async withdrawAllWithPid(@Body() withdrawAllWithPidDto: WithdrawAllWithPidDto) { // Assuming the existence of WithdrawAllWithPidDto
      try {
        ...
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            error: 'There was a problem with the withdraw all with pid operation in Wonder Staking',
            details: error.message,
          },
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
    ...
    @Post('compound')
    async compound(@Body() compoundDto: ClaimDto) { // compoundDto has the same format of ClaimDto thus reusing it.
        try {
