import {
  ...
  // Import DTOs later
import { RewardOfDto, DepositDto, WithdrawDto } from 'src/dto/liquid-staking-dto';
...
export class LiquidStakingController {
    ...
    async getTotalPooledWEMIXWithFee() {
        try {
            const result = await this.liquidStakingService.getTotalPooledWEMIXWithFee();
            return result;
        } catch(error) {
            ...
            throw new HttpException(
                {
                  status: HttpStatus.INTERNAL_SERVER_ERROR,
                  error:
                    'There was a problem getting rewardOf info in Liquid Staking',
                  details: error.message,
                },
                HttpStatus.INTERNAL_SERVER_ERROR,
              );
    ...
    async deposit(
      @Body() dto:DepositDto
        ...
        try {
            const result = await this.liquidStakingService.deposit(dto.msgSender,dto.amount);
            return result;
        } catch(error) {
            throw new HttpException(
                {
                  status: HttpStatus.INTERNAL_SERVER_ERROR,
                  error:
                    'There was a problem Depositing in Liquid Staking',
                  details: error.message,
                },
                HttpStatus.INTERNAL_SERVER_ERROR,
              );
    ...
    async withdraw(
      @Body() dto:WithdrawDto
        ...
        try {
            const result = await this.liquidStakingService.withdraw(dto.msgSender,dto.amount);
            return result;
        } catch(error) {
            throw new HttpException(
                {
