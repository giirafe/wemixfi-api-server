...
export class LiquidStakingService {
    ...
    async getTotalPooledWEMIXWithFee() {
        try {
            const totalWEMIX = await this.StWEMIXContract.getTotalPooledWEMIXWithFee();
    ...
    async deposit(
        ...
        try {
            ...
            const depositEvents = decodedLogs.filter((log) => log !== null && log.name === 'Deposited');
    
            const {wemixAmount,stWemixAmount} = depositEvents[0].args;
      ...
      async withdraw(
        ...
        // _amount = ethers.parseEther(_amount.toString())
        let amountInWei = await this.extendedEthersService.convertToWei(this.StWEMIXAddress, _amount)
        // value = _amount as bigint;
        ...
        try {
            ...
            const withdrawEvents = decodedLogs.filter((log) => log !== null && log.name === 'Withdrew');
            for (const withdrawEvent of withdrawEvents) {
              console.log(withdrawEvent);
            }
    
            const {wemixAmount,stWemixAmount} = withdrawEvents[0].args;
