...
import { SwapRouterHelper } from 'types/ethers';
...
export class SwapV3Service {
    ...
    async exactInput(
        ...
        const encodedBestPath = await this.findBestPath(tokenIn,tokenOut,amountIn)
        ...
        try {
            this.extendedEthersService.approveToken(tokenIn,senderWallet,amountIn,this.SwapRouterHelperAddress)
        } catch(error) {
            this.logger.error(
        ...
        try {
          ...
          if (tokenIn == this.wWemixAddress) { // In case of swapping wWemix (IN)
          ...
          const currentGasPrice = (await this.extendedEthersService.provider().getFeeData()).gasPrice
          ...
          const tx = await SwapRouterWithSigner.exactInput(exactInputParams, { value, gasPrice:higherGasPrice });

          const txReceipt = await tx.wait();
          const exactInputEvent = await this.extendedEthersService.getEventFromReceipt(
            txReceipt,
            'ExactInput',
          );
    ...
    async findBestPath(from: AddressLike, to: AddressLike, amountIn: BigNumberish): Promise<string> {
        const { data } = await firstValueFrom(
          this.httpService.get('https://devapi.wemix.fi/weswapV3/swapPath', {
            params: {
              from,
              to,
              amountIn,
